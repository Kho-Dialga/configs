...
local theme_path = string.format("%s/.config/awesome/themes/%s/theme.lua", os.getenv("HOME"), "dialga")
...
-- Torrent
function torrent()
	torrent_timer:emit_signal("timeout")
end
torrent_widget, torrent_timer = awful.widget.watch('torrent', 20)
...
globalkeys = gears.table.join(
    ...
    awful.key({ modkey, "Shift" }, "d", function () awful.spawn( "dmenu_run" ) end,
              {description = "open a dmenu prompt", group = "launcher"}),
